...
4-minute global write window:
  No two write actions (post reply, send DM, post comment) may occur within
...
def seconds_until_next_write():
    """Return seconds until the next write is allowed (0 if allowed now)."""
    state = _get_write_state()
